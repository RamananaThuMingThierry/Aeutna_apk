...
class AuthServices{
  ...
  Future<bool> signup({String? email, String? pass, String? nom,String? prenom, String? contact, String? adresse}) async{
    try{
      final result = await auth.createUserWithEmailAndPassword(email: email!, password: pass!);
      if(result.user != null){
        await DbServices().saveUser(UserM(id: result.user!.uid, email: email,nom: nom,prenom: prenom, contact: contact, adresse: adresse));
        return true;
      }else{
        return false;
      }
