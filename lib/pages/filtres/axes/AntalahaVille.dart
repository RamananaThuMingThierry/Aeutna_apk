...
class AntalahaVille extends StatefulWidget {
  const AntalahaVille({Key? key}) : super(key: key);

  @override
  State<AntalahaVille> createState() => _AntalahaVilleState();
}

class _AntalahaVilleState extends State<AntalahaVille> {
  ...
  getUsersStream() async{
    var data = await FirebaseFirestore.instance.collection("users").where("axes", isEqualTo: "Antalaha Ville").get();
    setState(() {
  ...
  Widget build(BuildContext context) {
   setState(() {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("Antalaha Ville"),
        centerTitle: true,
        backgroundColor: Colors.blueGrey,
        actions: [
          Chip(label: Text("${total}", style: TextStyle(color: Colors.white),)),
        ],
      ),
